...
contract Market is IMarket, Initializable {
    ...
    function initialize(
        ...
        _initAddresses(_marketInfo.creator, _oracle, _marketAMM, _usdc);
        _initFeeBPS(_marketInfo.feeBPS);

        _setMarketState(MarketState.open);

        _emitMarketInit(_marketInfo);
     ...
     */
    function addLiquidity(uint256 _amount, uint256 _deadline) external validDeadline(_deadline) {
     ...
     */
    function removeLiquidity(uint256 _shares, uint256 _deadline) external notClosed validDeadline(_deadline) {
        // Checks
        ...
        emit LiquidityRemoved(msg.sender, _shares, amount, outcomeSharesToReturn);
        ...
        validDeadline(_deadline)
    {
        ...
        _amount = USDC.balanceOf(address(this)) - balanceBefore;
        ...
        uint256[] memory newOutcomesPrice = new uint256[](2);
        newOutcomesPrice[0] = getOutcomePrice(0);
        newOutcomesPrice[1] = 1e6 - newOutcomesPrice[0];
        emit SharesBought(msg.sender, _outcomeIndex, _amount, fee, shares, newOutcomesPrice);
    }
     ...
     * @param _minReceiveAmount the minimum amount of ETH (before fee) to receive for shares
     * @param _deadline the deadline to sell shares
     */
    function sellShares(uint256 _shares, uint256 _outcomeIndex, uint256 _minReceiveAmount, uint256 _deadline)
        external
        notClosed
        validDeadline(_deadline)
    {
        // Checks
        if (userToOutcomeIndexToShares[msg.sender][_outcomeIndex] < _shares) {
            revert InsufficientShares();
        }

        (uint256 receiveAmount) = marketAMM.getSellOutcomeData(
            _shares,
            _outcomeIndex,
            IMarketAMM.MarketPoolState({liquidity: poolData.liquidity, outcomeShares: _getPoolShares()})
        );

        if (receiveAmount < _minReceiveAmount) {
            revert MinReceiveAmountNotMet();
        }

        // Effects
        uint256 fee = receiveAmount * fees.feeBPS / BPS;
        uint256 receiveAmountAfterFee = receiveAmount - fee;

        fees.poolWeight += fee;
        fees.totalFeesCollected += fee;
        poolData.balance -= receiveAmount;

        userToOutcomeIndexToShares[msg.sender][_outcomeIndex] -= _shares;
        poolData.outcomes[_outcomeIndex].shares.available += _shares;
        poolData.totalAvailableShares += _shares;

        for (uint256 i = 0; i < info.outcomeCount; ++i) {
            Shares storage outcomeShares = poolData.outcomes[i].shares;
            outcomeShares.total = outcomeShares.total - receiveAmount;
            outcomeShares.available = outcomeShares.available - receiveAmount;
            poolData.totalAvailableShares -= receiveAmount;
        }
        ...
        emit SharesSold(msg.sender, _outcomeIndex, receiveAmountAfterFee, fee, _shares, newOutcomesPrice);
    ...
    function resolveMarket() external onlyAtState(MarketState.closed) {
        ...
        emit MarketResolved(resolvedOutcomeIndex);
    }

    /**
     * @notice Claims the rewards for the winning outcome
     */
    function claimRewards() external onlyAtState(MarketState.resolved) {
        // Checks
        ...
        emit RewardsClaimed(msg.sender, shares, resolvedOutcomeIndex);
     ...
     */
    function claimLiquidity() external onlyAtState(MarketState.resolved) {
        // Checks
    ...
    /**
     * @notice Returns the full information about the market
     * @return info The information of the market
     */
    function getFullInfo() external view returns (MarketFullInfo memory) {
        MarketInfo memory _info = info;
        (string[] memory names,,) = _getOutcomes();
        return MarketFullInfo({
            question: _info.question,
            outcomeCount: _info.outcomeCount,
            closeTime: _info.closeTime,
            createTime: _info.createTime,
            closedAt: _info.closedAt,
            outcomes: names,
     ...
     */
    function getUserOutcomeShares(address _user) external view returns (uint256[] memory shares) {
        shares = new uint256[](info.outcomeCount);
        for (uint256 i = 0; i < info.outcomeCount; ++i) {
            shares[i] = userToOutcomeIndexToShares[_user][i];
        }
    }
     ...
     */
    function getOutcomePrice(uint256 _outcomeIndex) public view returns (uint256 price) {
        if (state == MarketState.resolved) {
            return _outcomeIndex == resolvedOutcomeIndex ? 1e6 : 0;
        }
     ...
     * @param _amount The amount of ETH to add to the market
     */
    function _addLiquidity(address _user, uint256 _amount) private notClosed {
        uint256 balanceBefore = USDC.balanceOf(address(this));
        USDC.safeTransferFrom(msg.sender, address(this), _amount);
        _amount = USDC.balanceOf(address(this)) - balanceBefore;
        ...
        emit LiquidityAdded(_user, _amount, liquidityShares, outcomeShareToReturn);
    }
    ...
    function _initMarketInfo(MarketInfoInput calldata _marketInfo) private {
        ...
        info.marketURI = _marketInfo.marketURI;
    ...
    function _initOutcomes(string[] calldata _outcomeNames) private {
        if (_outcomeNames.length != 2) {
            revert OnlyBinaryMarketSupported();
        }
     ...
     * @param _marketAMM The MarketAMM contract used for calculations
     * @param _usdc The USDC contract
     */
    function _initAddresses(address _creator, IOracle _oracle, IMarketAMM _marketAMM, address _usdc)
        private
     ...
     */
    function _emitMarketInit(MarketInfoInput calldata _marketInfo) private {
        uint256[] memory newOutcomesPrice = new uint256[](2);
        newOutcomesPrice[0] = 0.5e6;
        newOutcomesPrice[1] = 0.5e6;
        ...
        emit MarketInitialized(
            ...
            resolveDelay,
            fees.feeBPS,
            _marketInfo.outcomeNames,
            newOutcomesPrice
        );
    }

    /**
     * @notice Get the outcome names
     */
