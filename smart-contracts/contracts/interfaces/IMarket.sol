...
interface IMarket {
     ...
     * @param marketURI The URI of the market
     ...
     * @param closedAt The time when the market was closed
     * @param marketURI The URI of the market
     * @dev Takes 7 storage slots
     */
    struct MarketInfo {
        string question;
    ...
    /**
     * @notice Struct to store the full information about the market
     * @param question The question/name of the market
     * @param outcomeCount The number of outcomes for the market
     * @param closeTime The time after which the market will be closed for trading
     * @param createTime The time when the market was created
     * @param outcomes The outcome names
     * @param resolveDelay The delay after the close time to resolve the market
     * @param state The market state
     * @param oracle The address of the market oracle
     */
    struct MarketFullInfo {
        string question;
        uint256 outcomeCount;
        uint256 closeTime;
        uint256 createTime;
     ...
     * @param _marketURI The URI of the market
    ...
    event MarketInitialized(
        ...
        uint256 _feeBPS,
        string[] _outcomeNames,
        uint256[] _outcomePrices
    );
     ...
     * @param _outcomeShareToReturn The outcome shares received for adding liquidity when the market is unbalanced
     */
    event LiquidityAdded(
        address indexed _provider, uint256 _amount, uint256 _liquidityShares, uint256[] _outcomeShareToReturn
    );
     ...
     * @param _outcomeShareToReturn The outcome shares received for removing liquidity when the market is unbalanced
     */
    event LiquidityRemoved(
        address indexed _provider, uint256 _shares, uint256 _amount, uint256[] _outcomeShareToReturn
    );
     ...
     * @param _newOutcomePrice The new price of the outcomes
     */
    event SharesBought(
        address indexed _buyer,
        uint256 indexed _outcomeIndex,
        uint256 indexed _amount,
        uint256 _fee,
        uint256 _shares,
        uint256[] _newOutcomePrice
    );
     ...
     * @param _outcomeIndex The outcome index shares burned
     */
    event RewardsClaimed(address indexed _claimer, uint256 _amount, uint256 _outcomeIndex);
     ...
     * @param _outcomeIndex The resolved outcome index
     */
    event MarketResolved(uint256 _outcomeIndex);
     ...
     */
    function addLiquidity(uint256 _amount, uint256 _deadline) external;
     ...
     */
    function removeLiquidity(uint256 _shares, uint256 _deadline) external;
     ...
     */
    function buyShares(uint256 _amount, uint256 _outcomeIndex, uint256 _minOutcomeShares, uint256 _deadline) external;
     ...
     * @return info The information of the market
     */
    function getFullInfo() external view returns (MarketFullInfo memory info);
    ...
    /**
     * @notice Returns the shares of the user for all outcomes
     * @return shares of the user for all outcomes
     */
