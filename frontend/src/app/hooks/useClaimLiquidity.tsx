...
export const useClaimLiquidity = () => {
    ...
    async (data: ClaimRewardsData) => {
      try {
        ...
        try {
          ...
          throw new Error("Failed to claim liquidity");
        }
      } catch (error) {
        endTransaction();
    ...
    const handleReceiptChange = async () => {
      ...
      if (hash && isSuccess) {
        toastIdRef.current = toast(
          { title: "Successfully claimed liquidity", dismissable: false },
          { duration: 5 * 1_000 },
        );
        endTransaction();
        setTimeout(() => {
      ...
      if (hash && isLoading) {
        toastIdRef.current = toast(
          { title: "Claiming liquidity...", dismissable: false, loading: true },
          { duration: 999 * 1_000 },
        );
        return;
      }
