...
import { RemoveLiquidityData } from "@/lib/types";
...
export const useRemoveLiquidity = () => {
  ...
  const callData = useRef<RemoveLiquidityData | null>(null);
  ...
  const startWriteFlow = useCallback(
    async (data: RemoveLiquidityData, onSuccess?: () => void) => {
    ...
    const handleReceiptChange = async () => {
      ...
      if (hash && isSuccess) {
        toastIdRef.current = toast(
          { title: "Successfully removed liquidity", dismissable: false },
          { duration: 5 * 1_000 },
        );
      ...
      if (hash && isLoading) {
        toastIdRef.current = toast(
          { title: "Removing liquidity...", dismissable: false, loading: true },
          { duration: 999 * 1_000 },
        );
        return;
      }
