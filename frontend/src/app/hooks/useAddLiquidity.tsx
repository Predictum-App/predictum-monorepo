...
import { AddLiquidityData } from "@/lib/types";
...
export const useAddLiquidity = () => {
  ...
  const [flowState, setFlowState] = useState<
    "waitingApprove" | "approved" | "waitingAdd" | "added" | null
  >(null);
  const toastIdRef = useRef<string | number | null>(null);
  const callData = useRef<AddLiquidityData | null>(null);
  ...
  const startWriteFlow = useCallback(
    async (data: AddLiquidityData, onSuccess?: () => void) => {
    ...
    const handleReceiptChange = async () => {
      ...
      if (hashAction && isConfirmedAction) {
        toastIdRef.current = toast(
          { title: "Successfully added liquidity", dismissable: false },
          { duration: 5 * 1_000 },
        );
      ...
      if (hashAction && isConfirmingAction) {
        toastIdRef.current = toast(
          { title: "Adding liquidity...", dismissable: false, loading: true },
          { duration: 999 * 1_000 },
        );
        return;
      }
